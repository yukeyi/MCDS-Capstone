        ...
        if (args.use_lovasz):
            ...
            for line in range(5):
                for column in range(5):
                    ...
                    temp[:,:,2:target.shape[2]-2,2:target.shape[3]-2] =\
                        target[:,:,line:line+target.shape[2]-4,column:column+target.shape[3]-4]
                    if(line + column == 0):
                        mask = temp
                    else:
                        mask += temp
            loss = loss.mean()
            loss = (loss.double()*(args.augmentation*torch.tensor(((mask>5) * (mask<15)).astype("float64")).cuda()+1)).mean()
        optim.zero_grad()
        loss.backward()
        optim.step()
