...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        if (len(self.file_list[index].split("fullVal_patches/"))>1):
            image_name = self.file_list[index].split("fullVal_patches/")[1]
...
def get_loader(mode="val",img_list_sorted = None):
    loader = None
    if mode == "val":
        data_path = "/pylon5/ac5616p/baij/DeepMiner/fullVal_patches/"
        shuffle = False
        ...
        loader = DataLoader(dataset, shuffle=shuffle, batch_size=args.batch_size, drop_last=False)
